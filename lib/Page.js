...
class Page extends EventEmitter {
  ...
  async getHeapSnapshot(options) {
    const {
      path = null
    } = options;
    let result = '';
    this._client.on('HeapProfiler.addHeapSnapshotChunk', reportChunk);
    await this._client.send('HeapProfiler.takeHeapSnapshot');
    this._client.removeListener('HeapProfiler.addHeapSnapshotChunk', reportChunk);
    fs.writeFileSync(path, result, 'utf8');

    function reportChunk(event) {
      result += event.chunk;
    }
  }
