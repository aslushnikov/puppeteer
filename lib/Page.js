...
const {SessionTree} = require('./SessionTree');
...
class Page extends EventEmitter {
  ...
  constructor(client, target, ignoreHTTPSErrors, screenshotTaskQueue) {
    ...
    this._closed = false;
    this._client = client;
    this._sessionTree = new SessionTree(client);
    this._target = target;
    ...
    /** @type {!Map<!Puppeteer.CDPSession, Worker>} */
    this._workers = new Map();
    this._sessionTree.on(Events.SessionTree.SessionAttached, async (session, targetInfo) => {
      const worker = new Worker(session, targetInfo.url, this._addConsoleMessage.bind(this));
      this._workers.set(session, worker);
      this.emit(Events.Page.WorkerCreated, worker);
    });
    this._sessionTree.on(Events.SessionTree.SessionDetached, session => {
      const worker = this._workers.get(session);
      if (!worker)
        return;
      this._workers.delete(session);
    ...
    this._sessionTree.on('Runtime.exceptionThrown', (session, exception) => this._handleException(exception.exceptionDetails));
    this._sessionTree.on('Runtime.consoleAPICalled', (session, event) => this._onConsoleAPI(session, event));
   ...
   */
  async _onConsoleAPI(session, event) {
    ...
    let context = null;
    if (this._workers.has(session))
      context = await this._workers.get(session).executionContext();
    else
      context = this._frameManager.executionContextById(event.executionContextId);
