...
class Target {
  ...
  async crash() {
    ...
    // We might get command result, or might not get at all.
    const error = await Promise.race([
      Promise.all([
        session.send('Page.stopLoading'),
        session.send('Page.crash'),
      ]).then(() => null).catch(error => error),
      new Promise(resolve => session.once('Inspector.targetCrashed', resolve.bind(null, null)))
    ]);
    if (error)
      throw error;
  }
